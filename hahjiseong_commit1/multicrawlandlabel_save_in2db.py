...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--threads', dest='threads', default=5, type=int, help='쓰레드 개수를 입력하세요, 추천 : 5')
    parser.add_argument('-p', '--process', dest='process', default=multiprocessing.cpu_count(), type=int, help='프로세스 개수를 입력하세요, 추천 : default ')
    parser.add_argument('-d', '--days', dest='days', default= 0, type=int, help='며칠 뒤로 가고 싶으신가요?')
    parser.add_argument('-H', '--hours', dest='hours',default=0, type=int, help='몇시간 뒤로 가고싶으신가요?')
    parser.add_argument('-m', '--minutes', dest='minutes',default=10, type=int, help='몇분 뒤로 가고 싶으신가요?')
    parser.add_argument('-b', '--batch_size', dest='batch_size', default=4,type=int, help='batch_size number!')
    ...
    if way == 'timed':
        multicrawl = multicrawl_and_return(batch_size=batch_size,days=days,hours=hours,minutes=minutes,timed=True,num_thread=num_thread,num_process=num_process)
        ...
        multicrawl = multicrawl_and_return(batch_size=batch_size, days=days, hours=hours, minutes=minutes, timed=False,when=datetime_date,num_thread=num_thread,num_process=num_process)
        do_it = multicrawl.save_result2db()
        print(multicrawl.model.result)
    else:
